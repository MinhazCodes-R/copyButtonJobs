...
export default function Home() {
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', width: '300px', gap: '5px' }}>
        <button onClick={async () => await navigator.clipboard.writeText("Embedded Systems Intern")}>
          Embedded Systems Intern
        </button>
        <button onClick={async () => await navigator.clipboard.writeText("Heather Glen Village")}>
          Heather Glen Village
        </button>
        <button onClick={async () => await navigator.clipboard.writeText("May. 2025 -- Aug. 2025")}>
          May. 2025 -- Aug. 2025
        </button>
...
- Scripted a deployment pipeline that automated 90% of flashing and validation, reducing technician setup time from 45 minutes to under 5 minutes`
      ...
      {/* Web Developer */}
      <div style={{ display: 'flex', flexDirection: 'column', width: '300px', gap: '5px' }}>
        <button onClick={async () => await navigator.clipboard.writeText("Web Developer")}>
          Web Developer
        </button>
        <button onClick={async () => await navigator.clipboard.writeText("IEEE McMaster Student Branch")}>
          IEEE McMaster Student Branch
        </button>
        <button onClick={async () => await navigator.clipboard.writeText("Dec. 2024 -- Present")}>
          Dec. 2024 -- Present
        </button>
        <button onClick={async () => await navigator.clipboard.writeText(
...
- Engineered secure authentication with Prisma, Postgres, OAuth, and Bcrypt—streamlining logins and reducing password reset volume`
      ...
      {/* Software Team Lead */}
      <div style={{ display: 'flex', flexDirection: 'column', width: '300px', gap: '5px' }}>
        <button onClick={async () => await navigator.clipboard.writeText("Software Team Lead")}>
          Software Team Lead
        </button>
        <button onClick={async () => await navigator.clipboard.writeText("McMaster Aerial Robotics Team")}>
          McMaster Aerial Robotics Team
        </button>
        <button onClick={async () => await navigator.clipboard.writeText("Jan. 2025 -- Present")}>
          Jan. 2025 -- Present
        </button>
        <button onClick={async () => await navigator.clipboard.writeText(
...
- Developed and maintained team website with Next.JS and facilitated the migration to AWS`
      ...
      {/* Frontend Developer */}
      <div style={{ display: 'flex', flexDirection: 'column', width: '300px', gap: '5px' }}>
        <button onClick={async () => await navigator.clipboard.writeText("Frontend Developer")}>
          Frontend Developer
        </button>
        <button onClick={async () => await navigator.clipboard.writeText("Baitul Jannah Islamic Center (BJIC)")}>
          BJIC
        </button>
        <button onClick={async () => await navigator.clipboard.writeText("Jun. 2024 -- Sept. 2024")}>
          Jun. 2024 -- Sept. 2024
        </button>
        <button onClick={async () => await navigator.clipboard.writeText(
